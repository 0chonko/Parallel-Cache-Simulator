/*
	v.2
	IMPLEMENTS MERGE SORT (taskgroup with nested tasks)
*/
...
void TopDownSplitMerge(int *extra_space, int start, int end, int *v)
{
	...
	// {
		...
		#pragma omp task if (size > TASK_SIZE) firstprivate(extra_space, size, middle)
		{
			TopDownSplitMerge(v, start, middle, extra_space);
		}
		#pragma omp task if (size > TASK_SIZE) firstprivate(extra_space, size, middle)
		{
			TopDownSplitMerge(v, middle, end, extra_space);
		}
		#pragma omp taskwait
	// }
...
void vecsort(int **vector_vectors, int *vector_lengths, long length_outer)
{
	...
	// one time copy of the array
	memcpy(extra_space, vector_vectors, sizeof(int *) * length_outer);
	// allocate all the inner arrays
	for (i = 0; i < length_outer; i++)
	{
		length_inner = vector_lengths[i];
		extra_space[i] =
			(int *)malloc(length_inner * sizeof(int));
		memcpy(extra_space[i], vector_vectors[i],
			length_inner * sizeof(int));
	}
	
    #pragma omp parallel
	{
		#pragma omp single
		printf("num available threads: %d / %d\n", omp_get_num_threads(), num_threads); 
		// #pragma omp for nowait
		#pragma omp taskgroup 
		{
			#pragma omp for nowait
				for (i = 0; i < length_outer; i++) {
					// sort data from extra_space into v
					#pragma omp task 
					{
					TopDownSplitMerge(extra_space[i], 0, vector_lengths[i],
							vector_vectors[i]);
					}
				}
		}
...
int main(int argc, char **argv)
{
	...
	{
		for (int i = 0; i < vector_lengths[k]-1; i++) {
			if (vector_vectors[k][i] > vector_vectors[k][i+1]) {
				printf("Sort failed! %d %d\n", vector_vectors[k][i], vector_vectors[k][i+1]);
				break;
			}
		}
	}
